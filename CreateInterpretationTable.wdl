...
task GenerateVariantTable {
  ...
  command <<<
    ...
    if grep -q "MutectVersion" "vcf_header.txt"; then
      ...
      zcat ~{input_vcf} | java -jar /home/biodocker/bin/snpEff/SnpSift.jar filter "(isVariant( GEN[0].GT )) &  GEN[0].DP > 8  & (ANN[*].GENE in SET[0])" | eval $SNPSIFT_EXTRACTFIELDS > ~{sample_basename}.PANEL_ALL.tab    
    fi
    rm vcf_header.txt
